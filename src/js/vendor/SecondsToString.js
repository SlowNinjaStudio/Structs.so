...
export function secondsToString(seconds) {
  ...
  result += num_years > 0 ? num_years + ' years ' : '';
  result += num_days > 0 ? num_days + ' days ' : '';
  if (num_years == 0) { 
    result += num_hours > 0 ? num_hours + ' hours ' : '';
    if (num_days == 0) {
      result += num_minutes > 0 ? num_minutes + ' minutes ' : '';
      if (num_hours == 0) {
        result += num_seconds > 0 ? Math.round(num_seconds * 100) / 100 + ' seconds' : '';
      }
    }
  }
  return result;
