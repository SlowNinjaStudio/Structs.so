...
/* StructureDrain
 * Used for draining a structure, destroying it
 *
...
export class StructureDrain {
  constructor() {
    this.type = 'Structure Draining';
  ...
  getInputPrefix(){
    return '';
     // TODO fix in chain [MsgDrainStructure validate()] then update here
    ...
    for (let i = 0; i < result.events.length; i++ ) {
      for (let x = 0; x < result.events[i].attributes.length; x++) {
        switch(result.events[i].attributes[x].key) {
          case "performing_structure_drain_amount":
